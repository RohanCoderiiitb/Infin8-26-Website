...
export default function FaqSection() {
  ...
  return (
    <section className="simple-faq-section">
        ...
        <div className="faq-accordion">
          {faqs.map((faq, index) => (
            <div key={index} className="accordion-item">
              <button
                className={`accordion-question ${openIndex === index ? "open" : ""}`}
                onClick={() => handleToggle(index)}
              >
                <span className="question-icon">{faq.icon}</span>
                <span className="question-text">{faq.question}</span>
                <span className="toggle-indicator">{openIndex === index ? "−" : "+"}</span>
              </button>
              
              {openIndex === index && (
                <div className="accordion-answer-panel">
